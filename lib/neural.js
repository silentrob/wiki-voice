...
function Neuralizer(trees){
	...
	trees.forEach(function(tree){
    	tree.Subject.forEach(function(Subject){
			obj[Subject.body] = {};
			var attributes = FetchAttributes(Subject.attributes);
			console.log(attributes);
			for( attribute in attributes){
				obj[Subject.body][attributes[attribute]] = {};
			}
			if( tree.Predicate ){
				if( _is(tree.Predicate,'nouns') || _is(tree.Predicate,'verbs') )
					obj[Subject.body][tree.Predicate.body] = {};
					
					tree.Object.forEach(function(Object){
					// Appu Hingnikar ?
					console.log(JSON.stringify(Object));
					if( _is(Object,'prepositions') ){ // TODO: Replace with prepositional classes;
