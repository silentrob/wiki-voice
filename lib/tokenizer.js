...
// More names are welcome here.
function Orphanizer(jsObjs){
    if(!(jsObjs instanceof Array)){
        jsObjs = [jsObjs]
    }
    jsObjs.map(function(jsObj){
        if( jsObj.parent ){
            delete (jsObj.parent);
        }

        if( jsObj != undefined && ( jsObj.attributes != undefined || jsObj.body != undefined ) ){
            for( var i = 0; child = ((jsObj.attributes)?jsObj.attributes[i]:jsObj.body[i]) ; i++ ){
                delete child.parent;
                Orphanizer(child);
            }
        }
